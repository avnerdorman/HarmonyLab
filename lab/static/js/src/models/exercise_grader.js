define(["lodash", "app/utils/analyze"], function (_, Analyze) {
  /**
  ...
  var ExerciseGrader = function (settings) {
    ...
    this.analyzer = new Analyze(
      this.settings.keySignature
    ); /* for analysis-based grading */
  };
  ...
  _.extend(ExerciseGrader.prototype, {
    ...
    grade: function (definition, inputChords) {
      ...
      if (
        [
          ...
          "figured_bass",
          "figured_bass_pcs",
        ].includes(definition.exercise.type)
      ) {
        var analysis_types = Object.keys(definition.exercise.analysis.mode)
          .filter(function (key) {
            return definition.exercise.analysis.mode[key];
          })
          .filter(function (mode) {
            return supported.includes(mode);
          });
        var abbrev_switch =
          definition.exercise.analysis.mode.abbreviate_thoroughbass;
      }
      ...
      for (i = 0, len = problems.length; i < len; i++) {
        ...
        switch (definition.exercise.type) {
          ...
          case "figured_bass_pcs":
            result = this.figuredBassPcsMatch(
              expected_notes,
              actual_notes,
              abbrev_switch
            );
        ...
        if (func == "getNoteName") {
          if (expected.length == 1) {
            ...
            expected_result = eval(
              "this.analyzer." + func + "(expected[0],expected)" + prop
            );
          }
          if (delivered.length == 1) {
            delivered_result = eval(
              "this.analyzer." + func + "(delivered[0],delivered)" + prop
            );
          }
        } else {
          /* Possible change: run this once only per exercise? */
          expected_result = eval("this.analyzer." + func + "(expected)" + prop);
          delivered_result = eval(
            "this.analyzer." + func + "(delivered)" + prop
          );
        }
    ...
    analysisPcsMatch: function (expected, delivered, analysis_types) {
      ...
      var expected_pcs = expected.map(function (midi) {
        return midi % 12;
      });
      var i, len;
      for (i = 0, len = delivered.length; i < len; i++) {
    ...
    bassMatch: function (expected, delivered) {
      ...
      var i, len;
      for (
        i = 0, len = Math.min(1, delivered.length) /* bass only */;
        i < len;
        i++
      ) {
        midi = delivered[i];

        if (expected.indexOf(midi) === 0) {
          /* bass only */
          result.note[midi] = CORRECT;
          result.count[CORRECT].push(midi);
        } else {
    ...
    sopranoMatch: function (expected, delivered) {
      ...
      var i, len;
      for (
        i = Math.max(0, delivered.length - 1),
          len = delivered.length /* soprano only */;
        i < len;
        i++
      ) {
        midi = delivered[i];

        if (expected.indexOf(midi) === expected.length - 1) {
          /* soprano only */
          result.note[midi] = CORRECT;
          result.count[CORRECT].push(midi);
        } else {
